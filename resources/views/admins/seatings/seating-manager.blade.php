        ...
        <table>
          ...
          <tbody>
            ...
            <tr class="text-center">
                ...
                <td>{{$seating->pending}}</td>
                <td>{{$seating->working}}</td>
                <td>{{$seating->empty_table}}</td>
                <td>
      ...
      <!-- pagination -->
        <ul class="pagination flex float-right">
          <li class="page-item {{ ($seatings->currentPage() == 1) ? 'disabled' : '' }}">
              <a class="page-link" href="{{ $seatings->url(1) }}"><span aria-hidden="true">&laquo;</span></a>
          </li>
          @for ($i = 1; $i <= $seatings->lastPage(); $i++)
            <li class="page-item {{ ($seatings->currentPage() == $i) ? 'active' : '' }}">
              <a class="page-link" href="{{ $seatings->url($i) }}">{{ $i }}</a>
            </li>
          @endfor
            <li class="page-item {{ ($seatings->currentPage() == $seatings->lastPage()) ? 'disabled' : '' }}">
              <a class="page-link" href="{{ $seatings->url($seatings->lastPage()) }}"><span aria-hidden="true">&raquo;</span></a>
            </li>
        </ul>
    </div>
</section>
